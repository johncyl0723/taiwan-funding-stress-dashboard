# 台灣資金緊俏每日儀表板

公開市場研究用途的台灣短期資金監測工具。線上：<https://tw-funding-dashboard.pages.dev/>

版面分三部分：

1. **今天資金緊不緊** —— TSS 綜合指數、燈號、規則式摘要、AI 短評、走勢圖、相關新聞
2. **TSS 怎麼組成的** —— 依知識圖譜由上而下展開（外生驅動 → 央行政策 → 銀行間價格 → 票券與企業融資 → 合成與判讀），
   每個指標左圖右文，右欄固定分成「這個指標在量什麼」與「現在的數字說什麼」；月頻與週頻背景另置一帶
3. **台幣 vs 美元** —— 外幣通道，結構同第二部分

## 兩種文字，來源不同

畫面上的文字有兩個來源，分開標示，不可混淆：

| 區塊 | 來源 | 失效時 |
| --- | --- | --- |
| **規則式摘要**、各指標的「現在的數字說什麼」 | `scripts/lib/insight.ts` 依當日數據套用預寫規則產生，不經 AI | 永遠存在 |
| **AI 短評／新聞摘要** | GitHub Action 內呼叫一次 LLM，把生成結果存進 `dashboard.json`，頁面本身仍是純靜態 | 未設任何憑證或呼叫失敗時整段留空，其餘照常發布 |

AI 文字的 prompt 只餵入當日數據、被排除的子指標與新聞標題（或新聞稿全文），並要求「不得引入未提供的數字」
「不得提供投資建議」；新聞摘要另外要求把新聞內容當「資料」而非指令。輸出未經人工審閱，畫面上明確標示
模型名稱、生成時間與免責聲明。

### 只用 Claude Code 訂閱，不接計量計費的 API

`scripts/lib/commentary.ts` 只走一條路：**Claude Code 訂閱**（`CLAUDE_CODE_OAUTH_TOKEN` 有設定才會生成，
沒設定就整段跳過，規則式文字不受影響）。不接 Anthropic 或 OpenAI 的計量計費 API key，
所以這個功能本身不會產生任何 API 帳單。

這條路是官方支援的：`claude setup-token` 產生的 OAuth token 綁定 Pro/Max/Team/Enterprise 訂閱，
Anthropic 自己的 GitHub Actions 排程範例（Daily Report）就是同樣「非程式相關、定期產生摘要」的用法，
用訂閱額度執行而不計入 API 帳單。詳見 [code.claude.com/docs/en/authentication](https://code.claude.com/docs/en/authentication)
與 [code.claude.com/docs/en/github-actions](https://code.claude.com/docs/en/github-actions)。

1. 本機裝好 `claude` CLI 並登入你的 Pro/Max 帳號後，執行：
   ```bash
   claude setup-token
   ```
   瀏覽器授權完會印出一個一年期的 token。
2. 到 repo 的 **Settings → Secrets and variables → Actions** 新增 secret `CLAUDE_CODE_OAUTH_TOKEN`，貼上這個 token。
3. 選用：新增 repository variable `CLAUDE_CODE_MODEL`（預設 `haiku`，可設 `sonnet`／`opus`）。

Token 一年到期，屆時要重新跑一次 `claude setup-token` 換新的；也會在**閒置超過一個月**時失效，
每日排程本身就會讓它持續保鮮，除非你把排程停用一段時間。這個 token「只能發模型請求」，
不能建立 Remote Control 連線或讀 claude.ai connectors，外流風險比一般帳密低，但仍要當機密保管
（只放在 repo secret，不要出現在程式碼或 log 裡）。

**其他實作細節**：跑 `claude` CLI 需要 Node.js 22+（workflow 已設定），且**刻意不加 `--bare`**——
官方文件明講 bare 模式不讀 `CLAUDE_CODE_OAUTH_TOKEN`（只認 API key），加了會驗證失敗。
新聞摘要用 `--json-schema` 直接拿結構化的 `{official, media}`，不必再用正規表示式拆文字。

## 指標徽章：一眼看懂方向

第二部分每個指標卡與第六部分的月頻卡，右上角都有一顆徽章，把數字翻成「對台灣資金面代表什麼」。
邏輯集中在 `src/signals.ts`（純函式，測試在 `scripts/lib/signals.test.ts`），畫面上每張卡片也會印出這次判斷的依據與數字。

**量的指標 → 水位增加（▲）／減少（▼）／持平（▶）**

| 指標 | 怎麼判 | 持平區間 |
| --- | --- | --- |
| 外資買賣超 | 近 5 日淨買超＝台幣流入＝增加；淨賣超＝減少 | ±10,000 百萬元 |
| 央行 NCD 淨發行 | 方向相反：淨吸收＝央行收回台幣＝減少；淨釋出＝增加 | ±10,000 百萬元 |
| NCD 未到期餘額 | 較 5 個交易日前下降＝到期釋出＝增加；上升＝減少 | 餘額的 ±0.5% |
| 超額準備、準備貨幣 | 較上月增加＝增加 | 近 60 期月變動標準差的 0.25 倍 |
| 政府存款 | 方向相反：增加＝資金流向國庫＝減少 | 同上 |
| M1B、M2 年增率 | 年增率是「成長速度」，改標成長加速／放緩，不說水位增減 | 同上 |

**價的指標 → 寬鬆／偏鬆／中性／偏緊／緊縮**

| 指標 | 怎麼判 |
| --- | --- |
| TFSS、初級－次級利差 | 120 日 z-score，與 TSS 同一套門檻：±1.0 偏緊／偏鬆、±1.5 緊縮／寬鬆；±0.5 以上只標「中性・略偏」。被材料性檢定排除時標「資料不足」並引用排除原因 |
| O/N 走廊位置 | 直接看位置：5% 以下貼近下限＝寬鬆，15% 以上偏緊，30% 以上緊縮 |
| 隔拆、3M TAIBOR、TAIBOR 斜率 | 與近 120 日中位數的差距：±3 bp 內中性，±3 bp 以上偏緊／偏鬆，±10 bp 以上緊縮／寬鬆 |
| 五大銀行基準利率 | 較上月上調＝偏緊、下調＝偏鬆，±0.125 個百分點以上為明顯 |

**不判斷方向的指標 → 「參考指標」**：SOFR、名目利差、USD/TWD、TAIBIR 30–180D 斜率。它們不直接對應台幣資金水位或寬緊，
硬給方向反而誤導。票券週統計與 364 天期 NCD 標售是圖表面板，方向同樣不單純，目前未加徽章。

隔拆、3M TAIBOR、TAIBOR 斜率刻意不用 z-score：這幾個利率貼著政策利率走，120 日標準差只有 0.1–0.4 bp，
z-score 會把雜訊放大成訊號，跟材料性檢定排除它們的原因相同。代價是這幾張平常幾乎都是「中性」，只有真的脫離常態才會變色。

所有門檻是依歷史資料分布訂的經驗值，未經回測校準；歷史樣本不足 30 個交易日時標「資料不足」。

## 新聞與 AI 摘要

兩種來源可用的素材差很多，摘要方式因此不同 —— 這個不對稱在畫面上會明講：

| 來源 | 取得的內容 | 摘要依據 | 標示 |
| --- | --- | --- | --- |
| 央行新聞稿 RSS | 標題＋**完整新聞稿內文**（政府公開資訊） | 全文，可保留具體數字 | 「摘要依據新聞稿全文」 |
| Google News RSS | **只有標題** | 標題歸納出的主題 | 「摘要僅依據標題」（警示色） |

**媒體全文刻意不抓**：有版權與付費牆限制，而且外部網頁內容是提示詞注入的典型載體。
媒體標題與摘要都不參與任何指標計算，原始標題與連結一律保留在摺疊區供查閱。

摘要的 system prompt 明確要求把新聞內容當「資料」而非指令 —— 即使其中出現看似指示或
宣稱更高權限的文字，也只能當被引用的內容看待。摘要與資金行情短評是兩次獨立呼叫，
任一失敗都不影響另一個，也不影響資料更新。

## 燈號怎麼算

**綜合指標**＝五個日頻子指標各取 120 日 z-score 後等權平均，燈號以 5 日均值判定：

| 子指標 | 內容 | 材料性門檻（120 日標準差） |
| --- | --- | --- |
| TFSS | 90D TAIBIR 初級 − 3M TAIBOR | 0.25 bp |
| 初級－次級利差 | 90D TAIBIR 初級 − 次級 | 0.25 bp |
| TAIBOR 斜率 | 3M − 1W | 0.25 bp |
| O/N 走廊位置 | (隔拆 − 7 天期 NCD 利率) ÷ (重貼現率 − 7 天期 NCD 利率) | 0.01 |
| NCD 淨發行 | 近 5 個交易日合計 | 5,000 百萬元 |

門檻：5 日均 z ≥ 1.0 開始偏緊、≥ 1.5 明顯緊俏、≥ 2.0 異常壓力。

### 四個判讀修正器

1. **材料性檢定**：z-score 是尺度無關的，序列再平也算得出 2σ。實測 TAIBOR 斜率的 120 日標準差只有 0.08 bp、O/N 走廊位置只有 0.0026，若照樣等權納入，0.2 bp 的擺動就會主導頭條燈號。未達門檻的子指標會被排除，並在畫面與摘要中列出原因。
2. **絕對水準 guard**：純相對分位在升息週期會系統性誤報。TFSS 仍低於 30 bp 且 O/N 貼近走廊下限時，燈號最高只給「開始偏緊」。
3. **5 日均值**：燈號以 composite 的 5 日移動平均判定，當日值只顯示不判燈，避免單日雜訊。
4. **事件旗標**：稅期、除息旺季、季底年底、農曆年前、央行理監事會前後。旗標亮起時在摘要標註「屬季節性」，但**不**升級燈號。

### 兩種「沒有燈號」

- **待資料確認**：樣本不足 30 個交易日，或當日核心指標未同步定盤。
- **正常（市場無明顯波動）**：資料完整，但每個子指標的變異都低於材料性門檻。台灣短率由央行主導、極為黏著，長時間毫無變異是常態 —— 這種安靜必須與資料有問題分開，否則會把「市場很平靜」誤報成「工具壞了」。

## 已知限制

- **TFSS 不是純粹的資金鬆緊指標**。分子是票券初級市場（企業信用），減項是銀行間定盤利率，價差同時含信用溢酬與流動性溢酬，性質接近 TED spread。
- **政策升息不會、也不應該點亮燈號**。升息時 TFSS 兩隻腳同步上移，價差不變甚至收斂。回測已驗證 2022-03、2023-03、2024-03 三次升息皆未誤報。
- **門檻尚未經過真實壓力事件校準**。回測區間（2022 年起，受 TAIBOR 公告清單深度限制）不含任何真實資金壓力事件，分位數建議門檻只會把燈號固定點亮 20%／8%／2% 的時間，不是校準，因此維持常態近似初值。
- **外幣通道缺少換匯點**，只能給名目利差與匯率波動，算不出避險後的隱含台幣成本。臺灣銀行遠期牌告尚未接上。
- **公債附條件（RP）利率未接上**：櫃買中心拒絕自動抓取。
- **存放比未接上**：央行「重要金融指標」只提供 XLS／ODS／PDF，需要試算表解析器。

## 資料來源

| 層 | 指標 | 頻率 | 來源 |
| --- | --- | --- | --- |
| 外生 | 外資買賣超 | 日 | 證交所三大法人 JSON（逐日查詢） |
| 央行 | NCD 淨發行、未到期餘額、各天期發行利率、附買回旗標 | 日 | 央行公開市場操作公告內文 |
| 央行 | 貼放利率（走廊上限） | 事件 | 央行貼放利率頁 |
| 央行 | 364 天期 NCD 標售（投標倍數、得標利率） | 月 | 央行開標結果公告 |
| 銀行間 | 隔夜拆款利率 | 日 | 央行重要指標（分頁回補） |
| 銀行間 | TAIBOR 全天期 | 日 | 銀行公會每日＋月報 Big5 CSV |
| 票券 | TAIBIR 初級／次級全天期 | 日 | 集保歷史區間查詢 |
| 票券 | 承作／兌償量 | 週 | 集保票券保管結算交割週統計 |
| 外幣 | USD SOFR 30／90／180 日均 | 日 | NY Fed Markets Data API |
| 外幣 | USD/TWD 收盤匯率 | 日 | 央行外匯資訊（分頁回補） |
| 月頻 | 超額準備、準備貨幣、M1B／M2 年增率、政府存款、五大銀行基準利率 | 月 | 央行 OpenData CSV |

### 抓取實作重點

- **集保 TAIBIR**：官網只有「當日」頁面，改用歷史區間查詢（POST ＋ CSRF token ＋ session cookie）。`SYNCHRONIZER_TOKEN` 是一次性的，每個區段都必須重新 GET 表單頁；單次查詢上限約 170 天，長區間自動切段。
- **TAIBOR**：數字不在網頁文字中，而在公告附的 Big5 CSV。每日公告附近 40 天、月報附整月；公告清單以 `?page=N` 翻頁，日常更新只讀第 1 頁，回測依區間長度翻頁。
- **央行 NCD**：金額只在「公開市場操作資訊」公告，「存單申購發行資訊」沒有金額，需以標題過濾。解析範圍必須切到公告本文（`一、`～`檔案下載`）—— 央行每頁的左側選單都有「小規模附買回測試操作」，整頁比對會天天誤報附買回。
- **欄位一律依表頭文字定位**（`90`、`三個月`、`本期超額或不足`），官網改欄序時直接報錯，而非靜默取到錯誤天期。
- **編碼自動判斷**：銀行公會 CSV 是 Big5、央行 OpenData 是 UTF-8 BOM，以表頭關鍵字是否解碼成功決定。
- **日期一律台北時間**（UTC+8），避免 UTC 換日造成整批資料錯位一天。
- **降級策略**：TAIBIR 初級與 TAIBOR 為核心來源，缺任一則整批不發布；其餘缺漏只讓該欄位為空值，並在畫面標示讀取狀態。
- **歷史合併**：每次更新向官方要一整段區間（預設 200 天）而非單日，首次執行即可取得約 140 個交易日的統計基期。NCD 與外資只回補約 12 個交易日，合併時舊值不會被清空。

## 架構

純靜態網站，資料與程式碼放在 GitHub，公開網址發布在 Cloudflare（刻意不用 GitHub Pages 的
`github.io` 網址，避免公開連結一眼看出背後放在 GitHub 上）：

- **前端**：Vite + React，`npm run build` 產出的 `dist/` 由 GitHub Actions 部署到 **Cloudflare Pages**（`tw-funding-dashboard.pages.dev`）
- **資料更新**：GitHub Actions 排程（UTC `0 4 * * 1-5`，即台灣時間週一至週五 12:00）跑測試後執行 `scripts/refresh.ts`，寫回 `public/data/` 並自動 commit
- **文字**：規則式模板（`scripts/lib/insight.ts`）＋ 選用的 AI 短評／新聞摘要（`scripts/lib/commentary.ts`），見上方「兩種文字」
- **手動更新**：頁面右上角按鈕（見下），或到 repo 的 Actions 頁籤選 `更新資料並部署` → `Run workflow`

沒有伺服器、沒有資料庫、沒有登入機制。選用的 AI 短評／新聞摘要吃 Claude Code 訂閱額度，不接計量計費 API，
未設 `CLAUDE_CODE_OAUTH_TOKEN` 即不啟用；手動更新按鈕若要做成「原地觸發不跳頁」，需另外加一支 Cloudflare Worker，見下。

## 部署到 Cloudflare Pages

一次性設定，設定好之後 GitHub Actions 每次都會自動部署，不需要重複操作：

1. **建立 Pages 專案**（本機跑一次即可，之後都由 CI 接手）：
   ```bash
   npm install -g wrangler
   wrangler login
   wrangler pages project create tw-funding-dashboard --production-branch=master --force
   ```
   `--force` 是必要的——新版 wrangler 預設會把 `pages project create` 導去 Cloudflare 另一個「Workers with Assets」
   產品，不是傳統 Pages，網址也不會是 `.pages.dev`。之後的 `wrangler pages deploy` 不需要這個旗標。
2. **建立 Cloudflare API Token**：<https://dash.cloudflare.com/profile/api-tokens> → Create Custom Token →
   權限只給 **Account → Cloudflare Pages → Edit**，範圍越小越好。
3. 到 repo 的 **Settings → Secrets and variables → Actions**：
   - 新增 secret `CLOUDFLARE_API_TOKEN`，貼上第 2 步的 token
   - 新增 variable `CLOUDFLARE_ACCOUNT_ID`，值是 Cloudflare Dashboard 右側欄可以看到的帳號 ID

部署那一步用 [`cloudflare/wrangler-action`](https://github.com/cloudflare/wrangler-action) 執行
`wrangler pages deploy dist --project-name=tw-funding-dashboard`，跟本機手動部署是同一支指令。

## 手動更新按鈕

按鈕預設是**純連結**，連到 GitHub 的 Actions「Run workflow」頁面 —— 零風險、零基礎設施，多跳一次頁換一鍵觸發。

要做成「原地觸發，不跳頁」，需要一支小型代理伺服器幫忙：靜態網站沒有能力代管有權限的 GitHub token，
任何寫進前端 JS 的密鑰，任何人看原始碼或開發者工具就能拿走。程式碼在 `worker/`，用 Cloudflare Workers
免費方案（一天 10 萬次請求，一個按鈕用不完），部署步驟如下 —— **這些步驟只有你能做**：建帳號、產生
憑證、貼機密都需要你本人操作，我沒辦法代勞，也還沒有帳號可以先幫你跑過一次。

1. **建立範圍剛好的 GitHub token**：Settings → Developer settings →
   [Fine-grained tokens](https://github.com/settings/tokens?type=beta) → Generate new token。
   Repository access 選**只勾這個 repo**；Permissions 只給 **Actions: Read and write**，其餘全部
   No access。範圍越小，萬一外流的損害越小。
2. **建 Cloudflare 帳號並裝 wrangler**（免費，[dash.cloudflare.com](https://dash.cloudflare.com) 註冊）：
   ```bash
   npm install -g wrangler
   cd worker
   wrangler login
   wrangler secret put GITHUB_TOKEN   # 貼上第 1 步的 token
   wrangler deploy
   ```
   部署完會印出一個 `https://taiwan-funding-stress-refresh.<你的子網域>.workers.dev` 網址。
3. 到這個 repo 的 **Settings → Secrets and variables → Actions → Variables**，新增 repository variable
   `REFRESH_WORKER_URL`，值就是第 2 步印出的網址。
4. 重新 build 一次（下次排程跑完，或手動 `Run workflow`），按鈕就會從純連結變成原地觸發。

`worker/trigger-refresh.js` 做了兩層防濫用：只接受來自這個網站的請求（檢查 `Origin`），
以及讀 `dashboard.json` 的 `updatedAt` 做 10 分鐘冷卻，避免有人繞過按鈕直接洗 Worker 網址、
浪費你的 GitHub Actions 額度。**這支 Worker 我沒有帳號可以先測過**，部署後請自己按一次確認能觸發。

## 本機啟動

```bash
npm install
npm run dev
```

```bash
npm run refresh
```

```bash
npm test
```

本機要試 AI 短評，需先跑過 `claude setup-token`，再設環境變數：

```bash
CLAUDE_CODE_OAUTH_TOKEN=... npm run refresh
```

回測與門檻檢查（一次性工具，不在每日排程內；TDCC 逐段查詢會花數分鐘）：

```bash
npm run backtest -- --from=2022-01-01
```

加 `--full` 會把逐日序列一併寫入輸出檔（數 MB，僅供本機檢視）。

## 尚未實作

- 遠期匯率／換匯點、公債 RP 利率、存放比（見「已知限制」）
- 信用事件層（票券商保證餘額、銀行 CDS）：台灣無公開日頻資料，目前只能靠初級－次級利差間接觀察

---

本專案僅供公開市場研究與教育用途，不構成投資、融資或交易建議，亦不反映任何未公開的銀行流動性資料。
